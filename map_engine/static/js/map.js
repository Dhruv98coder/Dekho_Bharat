...
const NCR_BOUNDS=[[28.20,76.65],[29.10,77.85]]; // Delhi NCR operational area
const NCR_CENTER=[28.6139,77.2090];
const NCR_VIEW_PAD=0.05;
...
function isInsideNCR(lat,lon){
 return lat>=NCR_BOUNDS[0][0] && lat<=NCR_BOUNDS[1][0] && lon>=NCR_BOUNDS[0][1] && lon<=NCR_BOUNDS[1][1];
}
...
function locateMe(force=false){
 ...
 navigator.geolocation.getCurrentPosition(pos=>{
   if(!isInsideNCR(pos.coords.latitude,pos.coords.longitude)){
     stopLiveLocation();
     alert("Live location is available only inside Delhi NCR.");
     return;
   }
   updateLocation(pos);
   if(force)setFromLocation({...currentPosition,name:"Current location"});
   map.setView([pos.coords.latitude,pos.coords.longitude],15);
 },()=>alert("Location permission is needed for live navigation."),{enableHighAccuracy:true,timeout:10000,maximumAge:3000});
 if(watchId===null)watchId=navigator.geolocation.watchPosition(updateLocation,()=>{}, {enableHighAccuracy:true,maximumAge:1500,timeout:10000});
}
function updateLocation(pos){
 if(!isInsideNCR(pos.coords.latitude,pos.coords.longitude)){
   ...
   if(fromLocation?.name==="Current location"){fromLocation=null;document.getElementById("fromInput").value="";}
   return;
 }
...
function saveTripToHistory(route){
 try{
  if(!route||!toLocation)return;
  fetch("/api/trips/",{method:"POST",headers:{"Content-Type":"application/json","X-CSRFToken":csrfToken()},
   body:JSON.stringify({title:`Trip to ${toLocation.name||"destination"}`,origin:fromLocation?.name||"Current location",
    destination:toLocation.name||"Destination",mode,distance_km:route.distance_km,duration_minutes:route.duration_minutes,
    details:{source:"intelligent-map"}})
  });
 }catch(e){console.warn("Trip history could not be saved",e)}
}
async function findRoute(){
 ...
 window.lastRoutes=data.routes||[data.route];activeRoute=data.route||data.routes[0];drawRoutes(window.lastRoutes);showSummary(activeRoute);showSteps(activeRoute);saveTripToHistory(activeRoute);document.getElementById("startNavBtn").classList.remove("hidden");document.getElementById("routePanel").classList.remove("hidden");
...
/* Deep-link support: GoPlan home/cards can open IntelligentMap with a destination. */
(function initGoPlanDeepLink(){
  const params=new URLSearchParams(window.location.search);
  const to=params.get("to");
  const lat=Number(params.get("lat"));
  const lon=Number(params.get("lon"));
  if(to){ const input=document.getElementById("toInput"); if(input) input.value=to; }
  if(Number.isFinite(lat)&&Number.isFinite(lon)){
    const point={latitude:lat,longitude:lon,name:to||"Destination"};
    try{ toLocation=point; pinDestination(point); map.setView([lat,lon],15); fetchDestinationWeather(); }catch(e){}
  } else if(to){
    setTimeout(()=>{
      const box=document.getElementById("toSuggestions");
      geocodeInput(to,box,results=>{ if(results&&results[0]) selectPlace("toInput",results[0]); },true);
    },350);
  }
})();
