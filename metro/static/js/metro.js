...
async function findMetroRoute(){
  ...
  try{
    ...
    fetch("/api/trips/",{method:"POST",headers:{"Content-Type":"application/json","X-CSRFToken":csrfMatch?csrfMatch[1]:""},
      body:JSON.stringify({title:`Metro: ${d.from.name} to ${d.to.name}`,origin:d.from.name,destination:d.to.name,
        mode:"metro",distance_km:d.route.distance_km,duration_minutes:d.route.estimated_time_minutes,
        details:{source:"delhi-metro"}})}).catch(()=>{});
